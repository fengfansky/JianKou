 ...
 */
public class MediaStreamsRenderer implements GLSurfaceView.Renderer {
  private static final String TAG = "MediaStreamsRenderer";
  ...
  public MediaStreamsRenderer(VideoView surface) {
  ...
  private class YuvImageRenderer implements VideoRenderer.Callbacks {
        ...
        if (displayWidth > 1 && displayHeight > 1 &&
          ...
          if (scalingType == ScalingType.SCALE_ASPECT_FIT) {
            ...
            if (displayAspectRatio > videoAspectRatio) {
              float deltaX = (displayWidth - videoAspectRatio * displayHeight) / screenWidth;
              texRight -= deltaX;
              texLeft += deltaX;
            } else {
              float deltaY = (displayHeight - displayWidth / videoAspectRatio) / screenHeight;
              texTop -= deltaY;
              texBottom += deltaY;
            }
          }
    ...
    public void setSize(final int width, final int height) {
      Log.d(TAG, "ID: " + id + ". YuvImageRenderer.setSize: " + width + " x " + height);
  ...
  public static void setView(GLSurfaceView surface) {
    ...
    instance = new MediaStreamsRenderer(surface);
  }
  */
