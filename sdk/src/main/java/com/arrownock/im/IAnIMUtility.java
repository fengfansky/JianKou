...
//			}
	...
	void connectUtil(String host, int port, String initTopic, String initSenderName, String deviceId, String appKey) throws ArrownockException {
		synchronized (LOG_TAG) {
			...
			if (secureConnection) {
				//TODO 1 处理实例化参数DEFAULT_KEEPALIVE, deviceId, appKey不完整  2 处理setAutoReconnect()方法
				mqttAgent = new PahoAgent(host, port, true, getServerCert(), getClientCert(), getClientKey(), initSenderName, initTopic, mqttEvent, "BKS");
//				mqttAgent.setAutoReconnect(false);
			} else {
				//TODO 1 处理MqttId参数 2 处理setAutoReconnect()方法
				mqttAgent = new PahoAgent(host, port, initSenderName, initTopic,appKey , "mqttId", deviceId, mqttEvent);
//				mqttAgent.setAutoReconnect(false);
			}
		}
	}
