...
public class ANEmojiUtil {
    private static String PREFIX = "\\u";

    public static String emojiConvertToString(String message) {
        return convertEmojiToString(message);
    }

    public static String stringConvertToEmoji(String message) {
        String nativeString = ascii2Native(message);
        if(nativeString != null) {
        	nativeString = nativeString.replace("\\u]", "\\u");
        }
        return nativeString;
    }

    private static String convertEmojiToString(String message) {
        ...
        while (matcher.find()) {
            matcher.appendReplacement(buffer, Matcher.quoteReplacement(string2Unicode(message.toString().substring(
    ...
    private static String ascii2Native(String str) {
        ...
        while (index != -1) {
            ...
            emojiStr = str.substring(index, index + 3);
            if (emojiStr.lastIndexOf("]") == 2) {
                sb.append(emojiStr);
                begin = index + 3;
    ...
    private static char ascii2Char(String str) {
        ...
        if (!PREFIX.equals(str.substring(0, 2))) {
            throw new IllegalArgumentException("Ascii string of a native character must start with " + PREFIX + ".");
        }
